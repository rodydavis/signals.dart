...
export function activate(context: vscode.ExtensionContext) {
	...
	const installSkillsDisposable = vscode.commands.registerCommand('signals-dart.installSkills', async () => {
		...
		return vscode.window.withProgress({
			...
			try {
				...
				await vscode.workspace.fs.createDirectory(vscode.Uri.joinPath(workspaceFolder.uri, '.agents'));
				...
				vscode.window.showErrorMessage(`Failed to install skills: ${error?.message || error}`);
				return false;
			}
		});
	});
...
async function checkForSkillsInstallation(context: vscode.ExtensionContext) {
	const workspaceFolders = vscode.workspace.workspaceFolders;
	if (!workspaceFolders || workspaceFolders.length === 0) {
		return;
	}
	...
	const isOptedOut = context.globalState.get<boolean>(optOutKey, false);
	if (isOptedOut) {
		return;
	}
	...
	vscode.window.showInformationMessage(
		'Signals AI Developer Skills are available for this workspace. Would you like to install them to .agents/skills?',
		installButton,
		dontShowAgainButton
	).then(async (selection) => {
		if (selection === installButton) {
			await vscode.commands.executeCommand('signals-dart.installSkills');
		} else if (selection === dontShowAgainButton) {
			await context.globalState.update(optOutKey, true);
		}
	});
}

class SignalsExplorerProvider implements vscode.WebviewViewProvider {
		...
		webviewView.webview.onDidReceiveMessage(async (message) => {
			switch (message.command) {
				case 'installSkills':
					...
					const success = await vscode.commands.executeCommand('signals-dart.installSkills');
					if (success) {
